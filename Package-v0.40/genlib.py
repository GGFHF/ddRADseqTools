...
def mutate_sequence(seq, indelprob, maxindelsize, locusmaxmut, technique, min_seq_len, ressite1_seq, ressite2_seq):
    ...
    # initialize the number of indel attempts
    indels_attempts_number = 0

    # mutate the sequence without inner sequences of restriction sites
    while True:
        ...
        for i in range(seq_mutations_number):
            ...
            # there is an indel and is not a excessive number of indel attempts due to a short fragments
            if indelprob > random.random() and indels_attempts_number <= 10:

                # add 1 to indels_attempts_number
                indels_attempts_number += 1
                ...
                if technique == 'SE':
                    j = random.randrange(0, min(min_seq_len, length))
                elif technique == 'PE':
                    j = random.randrange(0, length - indelsize)

                # build the new sequence
                new_seq = old_seq[0:j] + old_seq[(j + indelsize):length]

                # if new sequence lenght is lower than min_seq_len, the new sequence is not valid => exit for to start other time
                if len(new_seq) < min_seq_len:
                    break
            ...
            # there is a SNP
                ...
                # get SNP position
                if technique == 'SE':
                    j = random.randrange(0, min_seq_len)
                elif technique == 'PE':
                    if random.randrange(1,3) == 1:
                        j = random.randrange(0, min_seq_len)
                    else:
                        j = random.randrange(length - min_seq_len - 1, length)
                ...
                new_seq = old_seq[0:j] + mutated_nucleotide + old_seq[(j + 1):length]

            # assign new sequence to the previous sequence for the next iteration
            old_seq = new_seq

        # if new sequence lenght is greater or equeal to min_seq_len and both restriction sites sequences
        # are not found => break the loop while to end the function
        if len(new_seq) >= min_seq_len and new_seq.upper().find(ressite1_seq.upper()) == -1 and new_seq.upper().find(ressite2_seq.upper()) == -1:
            break
...
def write_fragments_stats(fragstfile, intervals_dict, total_fragments_count, written_fragments_count, minfragsize, maxfragsize, title):
    ...
    # show OK message 
    print('The statistics can be consulted in the file {0}.'.format(get_file_name(fragstfile)))
    print('The CSV file {0} can be used to import data in a statistics program.'.format(get_file_name(csv_fragstfile)))
...
def plot_fragments_graphic(fragstfile, intervals_dict, title):
    ...
    try:
        ...
        print('The library numpy is not installed. The program will not plot the fragments distribution graphic.')
        return
    ...
    try:
        ...
        print('The library matplotlib is not installed. The program will not plot the fragments distribution graphic.')
        return
    ...
    # show OK message 
    print('The statistics graphic has been saved in the file {0}.'.format(get_file_name(graphic_file)))
...
def write_pcrdup_stats(dupstfile, stats_dict):
    ...
    # show OK message 
    print('The statistics can be consulted in the file {0}.'.format(get_file_name(dupstfile)))
    print('The CSV file {0} can be used to import data in a statistics program.'.format(get_file_name(csv_dupstfile)))
...
def plot_pcrdup_graphics(dupstfile, stats_dict):
    ...
    try:
        ...
        print('The library numpy is not installed. The program will not plot the statistic graphic.')
        return
    ...
    try:
        ...
        print('The library matplotlib is not installed. The program will not plot the statistic graphic.')
        return
    ...
    # show OK message 
    print('The statistics graphics have been saved in the files {0} and {1}.'.format(get_file_name(graphic_file_1), get_file_name(graphic_file_2)))
...
def plot_individuals_withoutdata_graphic(dupstfile, stats_dict):
    ...
    try:
        ...
        print('The library numpy is not installed. The program will not plot the individuals per locus without data graphic.')
        return
    ...
    try:
        ...
        print('The library matplotlib is not installed. The program will not plot the individuals per locus without data graphic.')
        return
    ...
    # show OK message 
    print('The graphic of the distribution of the individuals without data per locus has been saved in the file {0}.'.format(get_file_name(graphic_file)))
...
def plot_loci_withoutdata_graphic(dupstfile, stats_dict):
    ...
    try:
        ...
        print('The library numpy is not installed. The program will not plot the loci per individual without data graphic.')
        return
    ...
    try:
        ...
        print('The library matplotlib is not installed. The program will not plot the loci per individual without data graphic.')
        return
    ...
    # show OK message 
    print('The graphic of the distribution of the loci without data per individual has been saved in the file {0}.'.format(get_file_name(graphic_file)))
...
def write_GC_distribution(fragsfile, GC_distribution_dict):
    ...
    # show OK message 
    print('The file {0} containing the GC distribution has been created.'.format(get_file_name(GC_distribution_file)))
...
def get_all_options_dict():
    ...
    all_options_dict = {
        ...
        'clearfile': {'value':'', 'default':'./reads-cleared', 'comment':'path of the output file with PCR duplicates removed without extension'},
        ...
        'dupstfile': {'value':'', 'default':'./pcrduplicates-stats.txt', 'comment':'path of the the PCR duplicates statistics file'},
        ...
        'fragsfile': {'value':'', 'default':'./fragments.fasta', 'comment':'path of the fragments file'},
        ...
        'fragstfile': {'value':'', 'default':'./fragments-stats.txt', 'comment':'path of the output statistics file'},
        'gcfactor': {'value':'', 'default':'0.0', 'comment':'weight factor of GC ratio in a locus with PCR duplicates (0.0 <= gcfactor < 1.0)'},
        'genfile': {'value':'', 'default':'./genome.fasta', 'comment':'file of the reference genome in fasta format'},
        ...
        'readsfile': {'value':'', 'default':'./reads', 'comment':'path of the output file with generated sequences without extension'},
        'readsfile1': {'value':'', 'default':'./reads-1.fastq', 'comment':'path of the reads file in SE read type or the Watson strand reads file in PE case'},
        'readsfile2': {'value':'', 'default':'./reads-2.fastq', 'comment':'path of the Crick strand reads file in PE read type or NONE in SE case'},
        ...
        'seq': {'value':'', 'default':'TGGAGGTGGGG', 'comment':'sequence to be located into the genome'},
        ...
        'trimfile': {'value':'', 'default':'./reads-trimmed', 'comment':'path of the output file with trimmed reads without extension'},
   ...
   #---------------
    ...
    def __init__(self, code_exception, param1='', param2='', param3=''):
        ...
        if code_exception == 'D001':
            print('*** ERROR {0}: The {1} value must be an integer greater than {2}.'.format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'D002':
            print('*** ERROR {0}: The {1} value must be an integer greater or equal than {2} and lower or equal than {3}.'.format(code_exception, param1, param2, param3), file=sys.stderr)
        elif code_exception == 'D003':
            print('*** ERROR {0}: The {1} value contained by {2} is not a float number.'.format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'D004':
            print('*** ERROR {0}: The {1} value must be a real greater or equal than {2} and lower or equal than {3}.'.format(code_exception, param1, param2, param3), file=sys.stderr)
        elif code_exception == 'D005':
            print('*** ERROR {0}: The {1} value must be a real greater or equal than {2} and lower than {3}.'.format(code_exception, param1, param2, param3), file=sys.stderr)
        elif code_exception == 'D006':
            print('*** ERROR {0}: The {1} value must be a real greater or equal than {2}.'.format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'D101':
            print('*** ERROR {0}: Invalid pattern {1} in string {2}.'.format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'D102':
            print('*** ERROR {0}: Invalid pattern of record --->{1}<--- in file {2}.'.format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'D103':
            print('*** ERROR {0}: Invalid DNA sequence {1} in file {2}.'.format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'D201':
            print('*** ERROR {0}: {1} is an invalid parameter.'.format(code_exception, param1), file=sys.stderr)
        elif code_exception == 'D202':
            print('*** ERROR {0}: tecnique {1} is not avaiable.'.format(code_exception, param1), file=sys.stderr)
        elif code_exception == 'D203':
            print('*** ERROR {0}: format {1} of output file is wrong. It must be FASTA or FASTQ.'.format(code_exception, param1), file=sys.stderr)
        elif code_exception == 'D204':
            print('*** ERROR {0}: read type {1} is wrong. It must be SE or PE.'.format(code_exception, param1), file=sys.stderr)
        elif code_exception == 'D301':
            print('*** ERROR {0}: Enzyme identification {1} not found in {2}.'.format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'D302':
            print("*** ERROR {0}: The cut mark '*' is not found in the restriction site sequence {1}.".format(code_exception, param1), file=sys.stderr)
        elif code_exception == 'D303':
            print('*** ERROR {0}: End identification {1} not found in {2}.'.format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'D304':
            print('*** ERROR {0}: {1} sequence has not found in {2} end of the fragment {3}.'.format(code_exception, param1, param2, param3), file=sys.stderr)
        elif code_exception == 'D305':
            print("*** ERROR {0}: The index1 must be represented by one sequence {1} in at the 5' end of the Watson strand.".format(code_exception, param1), file=sys.stderr)
        elif code_exception == 'D306':
            print("*** ERROR {0}: The DBR must be represented by one sequence {1} in at the 5' end of the Watson or Crick strand.".format(code_exception, param1), file=sys.stderr)
        elif code_exception == 'D307':
            print("*** ERROR {0}: The index2 must be represented by one sequence {1} in at the 5' end of the Crick strand.".format(code_exception, param1), file=sys.stderr)
        elif code_exception == 'F001':
            print('*** ERROR {0}: {1} can not be created.'.format(code_exception, param1), file=sys.stderr)
        elif code_exception == 'F002':
            print('*** ERROR {0}: {1} can not be opened.'.format(code_exception, param1), file=sys.stderr)
            ...
            print('*** ERROR {0}: The length of {1} is not equeal to the length of {2}.'.format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'L002':
            print('*** ERROR {0}: {1} is not found in {2}.'.format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'L003':
            print('*** ERROR {0}: Reads number of file {1} is not equal to reads number of file {2}.'.format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'L004':
            print("*** ERROR {0}: A {1} is not used by the technique {2} in the file {3}.".format(code_exception, param1, param2, param3), file=sys.stderr)
        elif code_exception == 'L005':
            print("*** ERROR {0}: A {1} is required by the technique {2} in the file {3}.".format(code_exception, param1, param2, param3), file=sys.stderr)
        elif code_exception == 'L006':
            print("*** ERROR {0}: The identification of replicated individual {1} is not a identification of individual in the file {2}.".format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'L007':
            print("*** ERROR {0}: The identification of replicated individual {1} is a identification of other replicated individual in the file {2}.".format(code_exception, param1, param2), file=sys.stderr)
        elif code_exception == 'L008':
            print("*** ERROR {0}: {1} must be comma-separated float numbers and they must sum 1.0.".format(code_exception, param1), file=sys.stderr)
        elif code_exception == 'S001':
            print('*** ERROR {0}: OS not detected.'.format(code_exception), file=sys.stderr)
        elif code_exception == 'S002':
            print('*** ERROR {0}: Sorting of file {1} has mistakenly finished.'.format(code_exception, param1), file=sys.stderr)
        else:
