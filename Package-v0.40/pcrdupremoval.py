...
def process_pcr_duplicates(options_dict):
    ...
    if not trace():
        ...
        print('The temporal file {0} has been deleted.'.format(get_file_name(unified_reads_file)))
    ...
    if not trace():
        ...
        print('The temporal file {0} has been deleted.'.format(get_file_name(sorted_reads_file)))
    ...
    if not trace():
        ...
        print('The temporal file {0} has been deleted.'.format(get_file_name(purged_reads_file)))
...
def unify_records(format, readtype, readsfile1, readsfile2, unified_reads_file):
    ...
    # show OK message 
    print('\nThe temporal file {0} has been created.'.format(get_file_name(unified_reads_file)))
...
def sort_records(unified_reads_file, sorted_reads_file):
    ...
    print('Sorting the temporal file {0} ...'.format(get_file_name(unified_reads_file)))
    ...
    # show OK message 
    print('The temporal file {0} has been created.'.format(get_file_name(sorted_reads_file)))
...
def purge_records(format, readtype, sorted_reads_file, purged_reads_file):
    ...
    while record1 != '':
        ...
        if readtype == 'SE':
            ...
            if record1_seq1 != record2_seq1:
                ...
                stats_dict[key] = data_dict
                if trace(): print('record removed: {0}'.format(record1_info1))
    ...
    # show OK message 
    print('\nThe temporal file {0} has been created.'.format(get_file_name(purged_reads_file)))
...
def restore_format(format, readtype, purged_reads_file, clearfile1, clearfile2):
    ...
    # show OK message
    if readtype == 'SE':
        print('\nThe file {0} has been created.'.format(get_file_name(clearfile1)))
    elif readtype == 'PE':
        print('\nThe files {0} and {1} have been created.'.format(get_file_name(clearfile1), get_file_name(clearfile2)))
...
def print_help(options_dict):
    ...
    print('')
    print('{0} quantifies the PCR duplicates and removes them from a file(s) in FASTQ/FASTA format with sequences of a double digest RADseq.'.format(program_file))
    print('')
    ...
    print('       {0:12}   {1}'.format('--clearfile', options_dict['clearfile']['comment']))
    print('       {0:12}   {1}'.format('--dupstfile', options_dict['dupstfile']['comment']))
