...
def build_reads(options_dict):
    ...
    if trace(): print('Individuals num: {0}'.format(individuals_num))
    ...
    for data_fragment in fragments_list:
        ...
        if trace(): print('order: {0} - fragment_num: {1}'.format(order, fragment_num))
        ...
        if mutprob > 0:
            ...
            if trace(): print('The fragment of locus {0} has {1} mutation(s) sequence(s).'.format(loci_count, mutated_seq_num))
            if trace(): print('    Original sequence      : {0}'.format(fragment_seq))
            ...
            for i in range(mutated_seq_num):
                mutated_seq = mutate_sequence(fragment_seq, indelprob, maxindelsize, locusmaxmut, technique, (insertlen - len(ressite1_rcut_seq) - len(ressite2_lcut_seq)), ressite1_seq, ressite2_seq)
                mutated_seqs_list.append(mutated_seq)
                if trace(): print('    Mutated sequence {0}     : {1}'.format(i, mutated_seqs_list))
        ...
        while (True):
            ...
            if allele == 1:
                ...
                if trace(): print('    Individual: {0:11} - Prob. allele 1: {1:5f} - Random number: {2:5f} - 1er allele - is mutated?: {3:5} - seq: {4}'.format(individual_id, individual_allele1_probability, random_number, 'True' if individual_allele_ismutated else 'False', individual_allele_seq))
            else:
                ...
                if trace(): print('    Individual: {0:11} - Prob. allele 2: {1:5f} - Random number: {2:5f} - 2nd allele - is mutated?: {3:5} - seq: {4}'.format(individual_id, (1 - individual_allele1_probability), random_number, 'True' if individual_allele_ismutated else 'False', individual_allele_seq))
            ...
            for i in range(pcrdup_num + 1):
                ...
                if not trace(): sys.stdout.write('\rSimulated sequences reads written: {0:9d}'.format(total_reads_count))
    ...
    # show OK message 
    if readtype == 'SE':
        print('\nThe file {0} containing the simulated sequences has been created.'.format(get_file_name(readsfile1)))
    elif readtype == 'PE':
        print('\nThe files {0} and {1} containing the simulated sequences have been created.'.format(get_file_name(readsfile1), get_file_name(readsfile2)))
...
def print_help(options_dict):
    ...
    print('')
    print('{0} builds a file in FASTA/FASTQ format with simulated reads of a double digest RADseq.'.format(program_file))
    print('')
    ...
    print('')
    print('       {0:18}   {1}'.format('option', 'value'))
    print('       {0:18}   {1}'.format('=' * 14, '=' * 78))
    print('       {0:18}   {1}'.format('--fragsfile', options_dict['fragsfile']['comment']))
    print('       {0:18}   {1}'.format('--technique', options_dict['technique']['comment']))
    print('       {0:18}   {1}'.format('--format', options_dict['format']['comment']))
    print('       {0:18}   {1}'.format('--readsfile', options_dict['readsfile']['comment']))
    print('       {0:18}   {1}'.format('--readtype', options_dict['readtype']['comment']))
    print('       {0:18}   {1}'.format('--rsfile', options_dict['rsfile']['comment']))
    print('       {0:18}   {1}'.format('--enzyme1', options_dict['enzyme1']['comment']))
    print('       {0:18}   {1}'.format('--enzyme2', options_dict['enzyme2']['comment']))
    print('       {0:18}   {1}'.format('--endsfile', options_dict['endsfile']['comment']))
    print('       {0:18}   {1}'.format('--index1len', options_dict['index1len']['comment']))
    print('       {0:18}   {1}'.format('--index2len', options_dict['index2len']['comment']))
    print('       {0:18}   {1}'.format('--dbrlen', options_dict['dbrlen']['comment']))
    print('       {0:18}   {1}'.format('--wend', options_dict['wend']['comment']))
    print('       {0:18}   {1}'.format('--cend', options_dict['cend']['comment']))
    print('       {0:18}   {1}'.format('--individualsfile', options_dict['individualsfile']['comment']))
    print('       {0:18}   {1}'.format('--locinum', options_dict['locinum']['comment']))
    print('       {0:18}   {1}'.format('--readsnum', options_dict['readsnum']['comment']))
    print('       {0:18}   {1}'.format('--minreadvar', options_dict['minreadvar']['comment']))
    print('       {0:18}   {1}'.format('--maxreadvar', options_dict['maxreadvar']['comment']))
    print('       {0:18}   {1}'.format('--insertlen', options_dict['insertlen']['comment']))
    print('       {0:18}   {1}'.format('--mutprob', options_dict['mutprob']['comment']))
    print('       {0:18}   {1}'.format('--locusmaxmut', options_dict['locusmaxmut']['comment']))
    print('       {0:18}   {1}'.format('--indelprob', options_dict['indelprob']['comment']))
    print('       {0:18}   {1}'.format('--maxindelsize', options_dict['maxindelsize']['comment']))
    print('       {0:18}   {1}'.format('--dropout', options_dict['dropout']['comment']))
    print('       {0:18}   {1}'.format('--pcrdupprob', options_dict['pcrdupprob']['comment']))
    print('       {0:18}   {1}'.format('--pcrdistribution', options_dict['pcrdistribution']['comment']))
    print('       {0:18}   {1}'.format('--multiparam', options_dict['multiparam']['comment']))
    print('       {0:18}   {1}'.format('--poissonparam', options_dict['poissonparam']['comment']))
    print('       {0:18}   {1}'.format('--gcfactor', options_dict['gcfactor']['comment']))
