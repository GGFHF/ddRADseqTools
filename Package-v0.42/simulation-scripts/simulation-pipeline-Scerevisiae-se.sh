...
# simulates a double digest and generates their SE reads; it quantifies and
...
WEND=end63
CEND=end64
INDIVIDUALSFILE=individuals-5index1.txt
...
$DDRADSEQTOOLSDIR/indsdemultiplexing.py \
    ...
    --cend=$CEND \
    --individualsfile=$DDRADSEQTOOLSDIR/$INDIVIDUALSFILE \
    --readsfile1=$READSDIR/reads-cleared.fastq \
    --readsfile2=NONE \
    --verbose=YES \
    --trace=NO
if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
